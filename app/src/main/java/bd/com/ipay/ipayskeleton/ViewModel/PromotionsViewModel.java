...
public class PromotionsViewModel extends AndroidViewModel implements HttpResponseListener {
    ...
    public void claimPromotion(@Nullable Long outletId) {
        if (!Utilities.isConnectionAvailable(getApplication())) {
            Toaster.makeText(getApplication(), R.string.no_internet_connection, Toast.LENGTH_LONG);
        } else if (httpRequestClaimPromotionPostAsyncTask == null) {
            final PromotionClaimRequest promotionClaimRequest = new PromotionClaimRequest(claimedPromotion.getBusinessAccountId(), outletId);
            final String jsonRequestBody = gson.toJson(promotionClaimRequest);
            httpRequestClaimPromotionPostAsyncTask = new HttpRequestPostAsyncTask(Constants.COMMAND_PROMOTIONS_CLAIM, Constants.BASE_URL_OFFER + claimedPromotion.getRedeemPath(), jsonRequestBody, getApplication(), this, false);
